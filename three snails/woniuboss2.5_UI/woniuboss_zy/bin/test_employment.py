...
class EmployTest(unittest.TestCase):
	...
	@parameterized.expand(search__info)
	def test_search(self,stu_name,number,expect):
		search_data = {'name':stu_name,'stu_no':number}
		#执行搜索动作
		self.employ.search_name('..\\config\\base.conf',search_data)
		conten = self.driver.find_element_by_css_selector('#stuInfo_table1 > tbody:nth-child(2) > tr:nth-child(1) > td:nth-child(1)').text
		if '付文攀' in conten:
