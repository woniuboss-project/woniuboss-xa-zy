...
class EnterpriseTest(unittest.TestCase):
	...
	def test_search_enterprise(self,employee_name,expect):
		...
		sql_one = 'select count(enterprise_id) from enterprise_info where ent_name = "飞虎乐购"'
		sql_zero = 'select count(enterprise_id) from enterprise_info where ent_name = "中石油"'
		sql_all = 'select count(enterprise_id) from enterprise_info'
		count_one = Utility.query_one('..\\config\\base.conf', sql_one)
		count_zero = Utility.query_one('..\\config\\base.conf',sql_zero)
		if count_one[0] == 1:
			actual = 'query one'
		elif count_zero[0] == '0':
			actual = 'query zero'
